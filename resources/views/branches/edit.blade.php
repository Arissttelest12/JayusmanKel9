<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-slate-800">Edit Cabang</h2>
    </x-slot>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 max-w-2xl">
        ...
        <form action="{{ route('branches.update', $cabang->id_cabang) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            
            <div>
                <x-input-label for="nama_cabang" value="Nama Cabang" />
                <x-text-input id="nama_cabang" name="nama_cabang" type="text" class="block w-full" value="{{ $cabang->nama_cabang }}" required />
            </div>

            <div>
                <x-input-label for="kota" value="Kota" />
                <x-text-input id="kota" name="kota" type="text" class="block w-full" value="{{ $cabang->kota }}" required />
            </div>

            <div>
                <x-input-label for="alamat" value="Alamat" />
                <textarea id="alamat" name="alamat" rows="4" class="block w-full border-slate-300 focus:border-[#00ADB5] focus:ring-[#00ADB5] rounded-xl shadow-sm transition-all duration-300 ease-out hover:border-slate-400" required>{{ $cabang->alamat }}</textarea>
            </div>
