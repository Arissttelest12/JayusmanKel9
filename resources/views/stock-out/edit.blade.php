...
@section('header-title', 'Edit Stok Keluar')

@section('content')
<div class="bg-white p-6 rounded shadow max-w-lg">
    <h2 class="text-2xl font-bold mb-6">Form Edit Stok Keluar</h2>
    ...
    <form action="{{ route('stock-out.update', $stokKeluar->id_stok_keluar) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-4">
            ...
            <select name="id_cabang" id="id_cabang" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="{{ $cabang->id_cabang }}" {{ $stokKeluar->id_cabang == $cabang->id_cabang ? 'selected' : '' }}>{{ $cabang->nama_cabang }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="id_barang" id="id_barang" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="{{ $barang->id_barang }}" {{ $stokKeluar->id_barang == $barang->id_barang ? 'selected' : '' }}>{{ $barang->nama_barang }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="id_user" id="id_user" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="{{ $user->id }}" {{ $stokKeluar->id_user == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="jumlah" id="jumlah" value="{{ $stokKeluar->jumlah }}" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" min="1">
        </div>

        <div class="mb-4">
            <label for="tanggal_keluar" class="block text-sm font-medium text-gray-700">Tanggal Keluar</label>
            <input type="date" name="tanggal_keluar" id="tanggal_keluar" value="{{ $stokKeluar->tanggal_keluar->format('Y-m-d') }}" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>

        <div class="mb-6">
            <label for="alasan" class="block text-sm font-medium text-gray-700">Alasan</label>
            <textarea name="alasan" id="alasan" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $stokKeluar->alasan }}</textarea>
        </div>
