<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-slate-800">Tambah Stok Barang</h2>
    </x-slot>
        ...
        <form action="{{ route('stocks.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_stok" value="{{ old('jumlah_stok') }}" required min="0" 
                           class="w-full rounded-xl border-slate-300 focus:border-[#00ADB5] focus:ring-[#00ADB5]">
                </div>
            </div>

            <div class="flex justify-end space-x-3 pt-6 border-t border-slate-100">
                <a href="{{ route('stocks.index') }}" class="px-6 py-2.5 rounded-xl border border-slate-300 text-slate-600 font-semibold hover:bg-slate-50 transition-colors">
                    Batal
                </a>
