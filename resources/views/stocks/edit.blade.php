...
@section('header-title', 'Edit Stok Barang')

@section('content')
<div class="bg-white p-6 rounded shadow max-w-lg">
    <h2 class="text-2xl font-bold mb-6">Form Edit Stok</h2>
    ...
    <form action="{{ route('stocks.update', $stokBarang->id_stok) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-4">
            ...
            <select name="id_cabang" id="id_cabang" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="{{ $cabang->id_cabang }}" {{ $stokBarang->id_cabang == $cabang->id_cabang ? 'selected' : '' }}>{{ $cabang->nama_cabang }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="id_barang" id="id_barang" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <option value="{{ $barang->id_barang }}" {{ $stokBarang->id_barang == $barang->id_barang ? 'selected' : '' }}>{{ $barang->nama_barang }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="number" name="jumlah_stok" id="jumlah_stok" value="{{ $stokBarang->jumlah_stok }}" class="w-full px-3 py-2 border border-gray-300 rounded mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" min="0">
        </div>
